...
void build_support_graph_ns(ListDigraph& support_graph,
    ...
    for (NODE i : subG.nodes()) {
        pair_i_k.first = i;
        pair_i_k.second = k;
        if (xSol.at(pair_i_k) > (1 - TOL) &&
            v_nodes.count(i) == 0) {  // Mark change in 5.5
            a = support_graph.addNode();
            v_nodes[i] = a;
            rev_nodes[a] = i;
        }
    ...
    for (auto& arc : subG.arcs()) {
        ...
        if (xSol.at(pair_u_k) > (1 - TOL) &&
            xSol.at(pair_v_k) > (1 - TOL)) {  // Mark change in 5.5
...
bool separate_sc_ns(
    ...
    cout << "separate_sc_ns_count = " << ++separate_sc_ns_count << endl;
    bool ret = false;
    pair<NODE, INDEX> pair_i_k;
        ...
        if (components != 1) {
            ...
            for (auto& arc : subG.arcs()) {
                ...
                if (u_selected && !v_selected &&
                    nodemap[v_nodes[u]] == root_comp)
                    root_adj_nodes.insert(v);
                if (v_selected && !u_selected &&
                    nodemap[v_nodes[v]] == root_comp)
            ...
            for (auto i : root_adj_nodes) {
                ...
                newCutLhs += (partition_node_vars.at(pair_i_k));
                newCutValue += xSol.at(pair_i_k);
                cout << partition_node_vars.at(pair_i_k).getName() << " + ";
            }
            cout << endl;
...
bool seperate_min_cut_ns(
    ...
    for (auto k : G->p_set()) {
        ...
        build_cap_graph_ns(cap_graph, x_capacities, v_nodes, rev_nodes, xSol, G,
                           k, ns_root);
        ...
        for (auto q : subG.t_set()) {
            ...
            Preflow<ListDigraph, ListDigraph::ArcMap<double>> min_cut(
                cap_graph, x_capacities, v_nodes[ns_root.at(k)].first,
                v_nodes[q].first);
            min_cut.runMinCut();
            min_cut_value = min_cut.flowValue();
            ...
            if (min_cut_value < 1 - TOL) {
                ...
                /*for (ListDigraph::NodeIt i(cap_graph); i != INVALID; ++i)
                {
                        if (min_cut.minCut(i))
                        {
                                ...
                                if (is_cut_arc) {
                                        LOG << "find cur arc: " << rev_nodes[i]
                << endl; pair_i_k.first = rev_nodes[i]; newCutLhs +=
                (partition_node_vars.at(pair_i_k));
                                }
                        }
                }*/
